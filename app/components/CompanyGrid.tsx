...
const SECTOR_COLORS: Record<string, string> = {
  sigint: BRAND.info,
  'em-sca': BRAND.primary,
  'space-sigint': BRAND.accent,
};
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:items-center">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="t-muted" style={{ fontSize: '11px', fontFamily: 'var(--font-ui)' }}>
            Sector:
          </span>
          {['all', 'sigint', 'em-sca', 'space-sigint'].map((s) => (
            <button
              ...
              style={{
                borderColor:
                  sector === s ? (SECTOR_COLORS[s] ?? 'var(--theme-text-primary)') : 'var(--theme-border-strong)',
        ...
        {filtered.map((c) => (
          <div key={c.name} className="card" style={{ padding: 'var(--space-md)' }}>
            <div className="flex items-start justify-between gap-2" style={{ marginBottom: 'var(--space-sm)' }}>
              <div className="t-card-heading" style={{ fontSize: '14px', lineHeight: 1.3 }}>
                {c.name}
              </div>
              <span
                className="text-xs px-1.5 py-0.5 rounded border"
                style={{
                  borderColor: SECTOR_COLORS[c.sector],
            ...
            <div className="flex items-center justify-between text-xs" style={{ marginBottom: 'var(--space-sm)' }}>
              ...
              <span
                ...
                style={{ color: SECTOR_COLORS[c.sector], fontFamily: 'var(--font-ui)' }}
              >
                {formatRevenue(c.revenue)}
              </span>
            </div>
            <div className="t-muted" style={{ fontSize: '12px', lineHeight: 1.5, marginBottom: 'var(--space-sm)' }}>
              {c.focus}
            </div>
            <span
              className="text-xs px-2 py-0.5 rounded"
              style={{
                background: `${SECTOR_COLORS[c.sector]}15`,
                color: SECTOR_COLORS[c.sector],
                fontFamily: 'var(--font-ui)',
              }}
            >
