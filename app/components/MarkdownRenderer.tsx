...
interface RawItem { alias: string; section: string; label: string; subtitle: string; weekOffset: number; durationWeeks: number; }
...
function parseMermaidGantt(code: string) {
  ...
  for (const rawLine of code.split('\n')) {
    ...
    else if (afterAlias && aliasMap.has(afterAlias)) startWeek = aliasMap.get(afterAlias)!;
    else startWeek = 0;
...
function MermaidGanttBlock({ code }: { code: string }) {
  ...
  if (parsed.items.length === 0) {
    return <pre className="text-xs font-mono p-4 rounded-lg border overflow-x-auto" style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)', color: 'var(--text-secondary)' }}>{code}</pre>;
  }
    ...
    <>
      <div className="my-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>{parsed.items.length} tasks · {parsed.totalWeeks} weeks</div>
        </div>
        <div className="overflow-x-auto rounded-lg border p-4" style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)' }}>{chart}</div>
      </div>
      ...
      {open && (
        <div className="fixed inset-0" style={{ zIndex: 99999, background: 'var(--bg-base)' }}>
          <div className="flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: 'var(--border)', background: 'var(--bg-base)', position: 'sticky', top: 0, zIndex: 10 }}>
            <div>
              <span className="text-sm font-mono" style={{ color: 'var(--text-primary)' }}>Gantt Chart</span>
              <span className="text-xs font-mono ml-3" style={{ color: 'var(--text-muted)' }}>{parsed.totalWeeks} weeks · {parsed.items.length} tasks</span>
            </div>
        ...
        components={{
          ...
          code({ className, children, ...props }) {
            ...
            if (match && match[1] === 'mermaid' && typeof children === 'string') {
              ...
              return <pre className="text-xs font-mono p-4 rounded-lg border overflow-x-auto" style={{ borderColor: 'var(--border)', background: 'var(--bg-surface)', color: 'var(--text-secondary)' }}>{children}</pre>;
            }
