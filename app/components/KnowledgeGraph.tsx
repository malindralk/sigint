  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const filteredEdges = data.edges.filter(
        (e) =>
          filteredIds.has(e.source as string) &&
          filteredIds.has(e.target as string)
      );
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {["all", "em-sca", "sigint", "reference", "learning", "infrastructure"].map(
          (f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              ...
              style={{
                ...
                background:
                  filter === f ? `${COLORS[f] ?? "#e6edf3"}11` : "transparent",
              }}
            >
              {f}
            </button>
        ...
        {selected && (
          <div className="w-64 shrink-0 bg-bg-secondary border border-border-default rounded-lg p-4 space-y-3">
            <div>
              ...
              <div
                className="font-semibold text-text-primary text-sm leading-snug"
            ...
            <div>
              <span
                className="inline-block text-xs border rounded px-2 py-0.5 font-mono"
                style={{
                  borderColor: COLORS[selected.category],
                  color: COLORS[selected.category],
                }}
              >
                {selected.category}
              </span>
