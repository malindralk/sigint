      ...
      <ResponsiveContainer width="100%" height={300}>
        <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
          ...
          <XAxis type="number" dataKey="price" name="Price" scale="log" domain={[30, 30000]}
            tick={{ fill: BRAND.textMuted, fontSize: 10 }} axisLine={{ stroke: BRAND.borderSolid }}
            tickFormatter={v => v >= 1000 ? `$${v / 1000}k` : `$${v}`}
            label={{ value: 'Price (log scale)', position: 'insideBottom', offset: -10, fill: BRAND.textMuted, fontSize: 11 }} />
          <YAxis type="number" dataKey="bwMhz" name="Bandwidth"
            tick={{ fill: BRAND.textMuted, fontSize: 10 }} axisLine={{ stroke: BRAND.borderSolid }} tickFormatter={v => `${v} MS/s`} />
          <Tooltip contentStyle={TT} content={({ payload }) => {
            ...
            return (
              <div style={TT} className="p-3 space-y-1">
                ...
                <div>Price: <span style={{ color: BRAND.primary }}>${d.price.toLocaleString()}</span></div>
                <div>Bandwidth: <span style={{ color: BRAND.info }}>{d.bwMhz} MS/s</span></div>
                <div>ADC: <span style={{ color: BRAND.accent }}>{d.adcBits}-bit</span></div>
                <div>Freq: up to <span style={{ color: BRAND.accent }}>{d.freqGhz} GHz</span></div>
