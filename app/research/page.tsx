...
import { ATTACK_TAXONOMY, RESEARCH_PAPERS } from '@/lib/viz-data';
...
export default function ResearchPage() {
  const byYear = RESEARCH_PAPERS.reduce(
    (acc, p) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
      <div>
        ...
        >
          &gt; research_timeline / {RESEARCH_PAPERS.length} papers / 2021–2026
        </div>
          ...
          {ATTACK_TAXONOMY.map((cat) => (
            <div key={cat.name} className="card" style={{ padding: 'var(--space-md)' }}>
              <div
                ...
                style={{ fontSize: '13px', color: cat.color, marginBottom: 'var(--space-sm)' }}
              >
                {cat.name}
              </div>
              <ul className="space-y-2">
                {cat.children.map((a) => (
                  <li key={a.name}>
                    <div
                      className="t-body"
                      style={{ fontSize: '12px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)' }}
                    >
                      {a.name}
                    </div>
                    <div className="t-muted" style={{ fontSize: '11px', lineHeight: 1.4 }}>
                      {a.desc}
                    </div>
                  </li>
                ))}
        ...
        {years.map((year) => (
          <div key={year}>
            <div className="flex items-center gap-3" style={{ marginBottom: 'var(--space-sm)' }}>
              <span className="t-heading" style={{ fontSize: '16px' }}>
                {year}
              </span>
              <span className="t-muted" style={{ fontSize: '11px' }}>
                ({byYear[year].length} papers)
              </span>
              <div className="flex-1" style={{ borderTop: '1px solid var(--border)' }} />
            </div>
            <div className="space-y-2">
              {byYear[year].map((p) => (
                <div key={p.title} className="card" style={{ padding: 'var(--space-sm) var(--space-md)' }}>
                  <div className="flex flex-wrap items-start gap-2" style={{ marginBottom: 'var(--space-xs)' }}>
                    <span
                      ...
                      style={{
                        borderColor: VENUE_COLORS[p.venue] ?? 'var(--border)',
                        color: VENUE_COLORS[p.venue] ?? 'var(--text-muted)',
                    ...
                    <span
                      className="text-right shrink-0 sm:hidden t-muted"
                      style={{
                        fontSize: '11px',
                        fontFamily: 'var(--font-ui)',
                        color:
                  ...
                  <div className="flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="t-body" style={{ fontSize: '13px', color: 'var(--text-primary)' }}>
                        {p.title}
                      </div>
                    ...
                    <div className="text-right shrink-0 hidden sm:block">
                      ...
                      <div className="t-muted" style={{ fontSize: '10px' }}>
                        traces
                      </div>
                    </div>
                  </div>
