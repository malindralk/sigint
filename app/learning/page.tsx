...
export default function LearningPage() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
      <div>
        <div className="t-muted" style={{ fontSize: '10px', fontFamily: 'var(--font-ui)', marginBottom: 'var(--space-xs)' }}>&gt; learning_path / 26_weeks / {totalHours}h / {totalCourses}_courses</div>
        <h1 className="t-heading" style={{ color: 'var(--brand-primary)' }}>Learning Path</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="card" style={{ padding: 'var(--space-md)', textAlign: 'center' }}>
            <div className="t-stat" style={{ fontSize: '20px' }}>{s.value}</div>
          ...
          <div key={phase.phase} className="card" style={{ padding: 'var(--space-md)' }}>
            <div className="flex items-start justify-between" style={{ marginBottom: 'var(--space-sm)' }}>
              <div>
                ...
                <div className="t-muted" style={{ fontSize: '11px', fontFamily: 'var(--font-ui)', marginTop: 'var(--space-xs)' }}>
              ...
              <div className="progress-track" style={{ width: '64px', marginTop: 'var(--space-sm)' }}>
                <div className="progress-fill" style={{ width: `${(phase.hours / totalHours) * 100}%`, background: PHASE_COLORS[i] }} />
              </div>
            </div>
            <ul className="grid gap-1 sm:grid-cols-2">
              {phase.topics.map(t => (
                <li key={t} className="t-muted" style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
